...
export const recipes = {
	...
	trailmix: {
		...
		test: (cooker, names, tags) => {
			return (
				...
				tags.seed &&
				tags.seed >= 1 &&
				(names.berries || names.berries_cooked) &&
				tags.fruit &&
				tags.fruit >= 1 &&
				!tags.meat &&
				!tags.veggie &&
	...
	surfnturf: {
		...
		requirements: [
			TAG('meat', COMPARE('>=', 2.5)),
			TAG('fish', COMPARE('>=', 1.5)),
			NOT(TAG('frozen')),
		],
		priority: 30,
		foodtype: 'meat',
	...
	coffee: {
		...
		test: (cooker, names, tags) => {
			return (
				names.coffeebeans_cooked &&
				(names.coffeebeans_cooked === 4 ||
					(names.coffeebeans_cooked === 3 && (tags.dairy || tags.sweetener)))
			);
		},
		requirements: [
			OR(
	...
	tropicalbouillabaisse: {
		...
		test: (cooker, names, tags) => {
			return (
				(names.fish3 || names.fish3_cooked) &&
				(names.fish4 || names.fish4_cooked) &&
				(names.fish5 || names.fish5_cooked) &&
				tags.veggie
			);
		},
	...
	butterflymuffin_dst: {
		...
		requires: 'Butterfly Wings, veggie',
		requirements: [
			OR(NAME('butterflywings'), NAME('moonbutterflywings')),
			NOT(TAG('meat')),
	...
	unagi_dst: {
		...
		foodtype: 'veggie',
		health: healing_med,
		// Staying Afloat update value (previously 18.75 Hunger).
		hunger: calories_large,
		perish: perish_med,
		sanity: sanity_tiny,
		cooktime: 0.5,
	...
	mashedpotatoes: {
		...
		test: (cooker, names, tags) => {
			return (
				((names.potato && names.potato > 1) ||
					(names.potato_cooked && names.potato_cooked > 1) ||
					(names.potato && names.potato_cooked)) &&
				(names.garlic || names.garlic_cooked) &&
				!tags.meat &&
				!tags.inedible
			);
		},
		requirements: [
			NAME('potato', COMPARE('>', 1)),
			NAME('garlic'),
			NOT(TAG('meat')),
			NOT(TAG('inedible')),
		],
	...
	salsa: {
		...
		test: (cooker, names, tags) => {
			return (
				(names.tomato || names.tomato_cooked) &&
	...
	barnaclesushi: {
		...
		test: (cooker, names, tags) => {
			return (
				(names.barnacle || names.barnacle_cooked) &&
				(names.kelp || names.kelp_cooked) &&
				tags.egg &&
				tags.egg >= 1
			);
		},
		...
		hunger: calories_large,
		perish: perish_med,
		// Staying Afloat update value (previously 5 Sanity).
		sanity: sanity_med,
		cooktime: 0.5,
		mode: 'together',
	},
	barnaclinguine: {
		...
		priority: 30,
		foodtype: 'meat',
		// Staying Afloat update value (previously 10 Health).
		health: healing_medlarge,
	...
	koalefig_trunk: {
		...
		foodtype: 'meat',
		health: healing_huge,
		// Staying Afloat update values (previously 56.25 Hunger and 0 Sanity).
		hunger: calories_superhuge,
		sanity: sanity_tiny * 3,
		perish: perish_slow,
		cooktime: 2,
		mode: 'together',
	},
	...
	figkabab: {
		...
		foodtype: 'meat',
		health: healing_med,
		// Staying Afloat update values (previously 25 Hunger and 15 Sanity).
		hunger: calories_large,
		sanity: sanity_small,
		perish: perish_slow,
		temperature: hot_food_bonus_temp,
	...
	frognewton: {
		...
		priority: 1,
		foodtype: 'meat',
		// Staying Afloat update values (previously 8 Health, 18.75 Hunger, and 10 Sanity).
		health: healing_med,
		hunger: calories_med,
		sanity: sanity_tiny,
		perish: perish_slow,
	...
	potatosouffle: {
		...
		test: (cooker, names, tags) => {
			return (
				((names.potato && names.potato >= 2) ||
					(names.potato_cooked && names.potato_cooked >= 2) ||
					(names.potato && names.potato_cooked)) &&
				tags.egg &&
				!tags.meat &&
				!tags.inedible
	...
	bonesoup: {
		...
		requirements: [
			NAME('boneshard', COMPARE('=', 2)),
			NAME('onion'),
			TAG('inedible', COMPARE('<', 3)),
		],
		priority: 30,
		foodtype: 'meat',
